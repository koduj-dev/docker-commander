...
- **Resources** — one page for *how much does it take*: live CPU / memory / network per **container** and per **stack** (sortable, filterable, refreshed every 5 s), the network **top talkers** over a stored window, and a **Disk** tab that says which image, container, volume or build-cache record occupies the space (an image's unique size vs. what it shares) and what is reclaimable. See [Resources](docs/resources.md).
...
- **Data retention** — the alert feed, its delivery records, the audit log and project revisions are trimmed by a daily purge with a per-area TTL (on by default: 90 days for alerts, a year for the audit log — never under 30 days — and the newest 50 revisions per project), with a **Purge now** button and a record of what each run deleted. See [Settings → Data retention](docs/settings.md#data-retention).
...
the ceiling. Alongside **~1100 Go unit tests** and **~380 frontend tests**, the repo
