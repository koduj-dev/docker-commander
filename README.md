...
- **Domain mappings** — record that a domain should route to one of a project's services (validated as a real, globally-unique FQDN); config only for now, laying the groundwork for a future embedded reverse proxy — nothing routes traffic yet.
