...
- **Binary/data files in Projects** — the project file editor uploads text only (`writeProjectFile` sends `file.text()`). Add a raw octet-stream upload (like the container/volume uploader) and show non-text files as download-only in the tree, so datasets/binaries can live alongside the compose file. _(The volume & container file browsers already handle binary files.)_
- **Self-update / update notifier** — check the GitHub Releases API (`/repos/koduj-dev/docker-commander/releases/latest`) against the running version and surface an **update-available** banner (admin) + link. Then a **`dockercmd --self-upgrade`** command (later an in-app one-click + restart) that downloads the right OS/arch asset, **verifies a checksum/signature**, and atomically replaces the binary — via a maintained lib (`minio/selfupdate` or `creativeprojects/go-selfupdate`, which handle the Windows running-`.exe` rename). Phases: (1) notifier, (2) CLI upgrade, (3) in-app one-click. _Caveats: the binary must be writable; `release.yml` must publish per-OS/arch assets + a checksums file; verify downloads since self-update executes code (supply-chain)._
