...
- **Project templates — catalog, user-saved & parameterized.** Today's new-project templates are **4 hardcoded in the frontend** (`web/src/lib/projectTemplates.ts`); the UI just `writeProjectFile`s each file after create. The backend doesn't know about them, they're fixed, and there are no variables. Build it out in three phases:
  - **(C) Backend catalog (foundation).** Move templates server-side behind an API (`GET /api/project-templates` → `{id,name,description,files[]}`), shipped via `go:embed` (like `internal/service`), and grow the built-in set (Redis, MySQL + phpMyAdmin, Traefik, a monitoring stack…). The picker fetches instead of hardcoding. _Unlocks A and B._
  - **(A) Save as template.** Capture an existing project's files as a reusable template stored per-install (new `project_templates` table, or `DataDir/templates/<slug>/`, like parse-rules/webhooks) → appears in the picker alongside built-ins; export/import as `.zip`. _Biggest real gap for teams._
  - **(B) Parameterized templates.** Templates carry placeholders (`{{.ProjectName}}`, `{{.Port}}`, generated secrets) filled by a small form at create time and rendered with `text/template` (same mechanism as webhook bodies / the launchd plist). Combines with A. _Mind: render server-side, validate inputs, and don't leak unset-var errors into the seeded files._
